...
- **Roles:** ynascime worked on the maze generation core (`mazegen`,
  the recursive backtracker, the playable-board pass, and the BFS
  solver) and the config parser; luccaval worked on the output file
  writer and the packaging/licensing setup. The interactive terminal
...
- **Planning:** we started from the mandatory part end to end
  ...
  we always had something the Moulinette/`maze_analyzer.py` could check
...
**AI usage:** AI assistance was used to review the maze-generation logic
against the subject's validity rules (connectivity, wall coherence, the
"42" isolation exception), to design and write the interactive terminal
menu (`menu.py`) and the block-style coloured renderer (`visualizer.py`),
and to draft this README. All generated code was read, tested against
`maze_analyzer.py` and `flake8`/`mypy --strict`, and understood before
